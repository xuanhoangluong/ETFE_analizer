# -*- coding: utf-8 -*-
"""
Created on Sat Mar 21 15:05:14 2020
...
def main():
	...
	loc = int((height_amophous-1)/2)
	tank = expon.rvs(loc=loc, scale=30, size=5000)
	...
	for i in tfloop_net:
		...
		for z in range(0, restrict_under+1):
			if z % 2 == 1:
				temp['x0'] = temp_net[0][ix][0] - temp_net[2] + 1
			else:
				temp['x0'] = temp_net[0][ix][0] - temp_net[2]
			temp['x1'] = temp_net[0][ix][1]
			temp['x2'] = z
			temp['vec'] = []
		...
		for z in range(restrict_under, -1, -1):
			if z % 2 == 1:
				temp['x0'] = temp_net[0][ix][0] - temp_net[2] + 1
			else:
				temp['x0'] = temp_net[0][ix][0] - temp_net[2]
		...
		for z in range(height_poly, restrict_above-1, -1):
			if z % 2 == 1:
				temp['x0'] = temp_net[1][ix][0] - temp_net[3] + 1
			else:
				temp['x0'] = temp_net[1][ix][0] - temp_net[3]
			temp['x1'] = temp_net[1][ix][1]
			temp['x2'] = z
			temp['vec'] = []
			sites[temp['x0']][temp['x1']][temp['x2']] = 1
			temp_chain.append(dict(temp))
		...
		for z in range(restrict_above, height_poly + 1):
			if z % 2 == 1:
				temp['x0'] = temp_net[1][ix][0] - temp_net[3] + 1
			else:
				temp['x0'] = temp_net[1][ix][0] - temp_net[3]
	...
	while n <= amo_sites:
		...
		if switch == 0:
			...
			temp_seed['x0'] = temp_net[0][l_rand][0]
			temp_seed['x1'] = temp_net[0][l_rand][1]
			temp_seed['x2'] = restrict_under
			for z in range(0, restrict_under+1):
				if z % 2 == 1:
					temp['x0'] = temp_net[0][l_rand][0] - temp_net[2] + 1
				else:
					temp['x0'] = temp_net[0][l_rand][0] - temp_net[2]
				temp['x1'] = temp_net[0][l_rand][1]
				temp['x2'] = z
				temp['vec'] = []
				sites[temp['x0']][temp['x1']][temp['x2']] = 1
				temp_chain.append(dict(temp))
			del temp_net[0][l_rand]
			switch = 1
		elif switch == 1:
			l_rand = rd.randint(0, len(temp_net[1])-1)
			temp_seed['x0'] = temp_net[1][l_rand][0]
			temp_seed['x1'] = temp_net[1][l_rand][1]
			temp_seed['x2'] = restrict_above
			for z in range(height_poly, restrict_above-1, -1):
				if z % 2 == 1:
					 temp['x0'] = temp_net[1][l_rand][0] - temp_net[3] + 1
				else:
					 temp['x0'] = temp_net[1][l_rand][0] - temp_net[3]
				temp['x1'] = temp_net[1][l_rand][1]
				temp['x2'] = z
				temp['vec'] = []
				sites[temp['x0']][temp['x1']][temp['x2']] = 1
				temp_chain.append(dict(temp))
			del temp_net[1][l_rand]
			switch = 0
		temp_seed['vec'] = []
		...
		while True:
			temp_lengh = int(tank[rd.randint(0, len(tank)-1)])
			if (int((height_amophous-1)/2) <= temp_lengh <= int((height_amophous-1)*2.5)):
				break
		print('max lengh ' + str(temp_lengh))
		count = 0
		while count <= int((height_amophous-1)*2.5):
					...
					for i in range(len(temp_net[0])):
						if next_head[x]['x0'] == check_head[i][0] and \
						   next_head[x]['x1'] == check_head[i][1]:
							for z in range(restrict_under, -1, -1):
								if z % 2 == 1:
									 temp['x0'] = temp_net[0][i][0] - temp_net[2] + 1
								else:
									 temp['x0'] = temp_net[0][i][0] - temp_net[2]
								temp['x1'] = temp_net[0][i][1]
								temp['x2'] = z
								sites[temp['x0']][temp['x1']][temp['x2']] = 1
								temp_chain.append(dict(temp))
							del temp_net[0][i]
					...
					for i in range(len(temp_net[1])):
						if next_head[x]['x0'] == check_head[i][0] and \
						   next_head[x]['x1'] == check_head[i][1]:
							for z in range(restrict_above, height_poly + 1):
								if z % 2 == 1:
									 temp['x0'] = temp_net[1][i][0] - temp_net[3] + 1
								else:
									 temp['x0'] = temp_net[1][i][0] - temp_net[3]
								temp['x1'] = temp_net[1][i][1]
								temp['x2'] = z
								sites[temp['x0']][temp['x1']][temp['x2']] = 1
								temp_chain.append(dict(temp))
							del temp_net[1][i]
	...
	print(len(loop[0]),   len(loop[0])/len(list_chains_length))
	print(len(tail[0]),   len(tail[0])/len(list_chains_length))
	print(len(bridge[0]), len(bridge[0])/len(list_chains_length))
	print(chain_num)
...
## Analytic data
	...
	file.write("%10i loops %10.2f\n" % (len(loop[0]), len(loop[0])/len(list_chains_length)*100))
	...
	file.write("\n\n")
	file.write("%10i tails %10.2f\n" % (len(tail[0]), len(tail[0])/len(list_chains_length)*100))
	...
	file.write("\n\n")
	file.write("%10i bridges %10.2f\n" % (len(bridge[0]), len(bridge[0])/len(list_chains_length)*100))
